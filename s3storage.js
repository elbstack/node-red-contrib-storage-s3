...
function prepopulateFlows(resolve) {
    ...
    s3.getObject(params, function (err, doc) {
        if (err) {
            var promises = [];
            console.log(`>> Default flow: ${__dirname}/defaults/flow.json`);
...
var s3storage = {
    init: function (_settings) {
        ...
        s3Endpoint = settings.awsS3Endpoint ? new AWS.Endpoint(settings.awsS3Endpoint) :  null;
        appname = settings.awsS3Appname || require('os').hostname();
        AWS.config.region = settings.awsRegion || 'eu-west-1';

        return when.promise(function (resolve, reject) {
            s3 = new AWS.S3({endpoint: s3Endpoint});
            if (!s3BucketName) {
